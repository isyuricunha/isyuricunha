...
| 4 | Generate quote via Ella | `isyuricunha/ella@v1.10.0` (composite action). Fails gracefully with `continue-on-error: true` so the fallback can run. Requires `ELLA_APP_*` and `ELLA_AI_*` secrets. |
